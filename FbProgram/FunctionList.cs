	...
	internal class FunctionList
	{
		...
		public List<BandInfo> getBandList(List<BandInfo> preList)
		{
			...
			try
			{
				...
				foreach (IWebElement webElement in this.util.findElements("[class='flexList']"))
				{
					string text = "";
					try
					{
		...
		public List<BandInfo> getBandListFromQuery()
		{
			...
			try
			{
				...
				int num = int.Parse(this.util.findElement("[class='result']").Text);
				num = (num > this.band_findCnt ? this.band_findCnt : num);
				IReadOnlyCollection<IWebElement> webElements = this.util.findElements("[class='cCoverList _bandListContainer'] li");
				for (int i = webElements.Count<IWebElement>(); i < num; i = webElements.Count<IWebElement>())
				{
					this.util.ScrollDown(100, 500);
					webElements = this.util.findElements("[class='cCoverList _bandListContainer'] li");
				}
				...
				for (int j = 0; j < num; j++)
				{
					...
					string text = this.util.findElement(webElement, "[class='name'] [class='text']").Text;
					string attribute = this.util.findElement(webElement, "[class='bandUri _goBand']").GetAttribute("data-band_no");
		...
		public bool login(string id, string pw, string type, bool logout = true)
		{
			...
			try
			{
				...
				this.util.goToUrl("https://www.facebook.com/", 300);
				while (this.util.findElement("[data-testid='royal_login_form']") == null && this.util.findElement("[class='registration_redesign']") == null)
				{
					this.util.delay(500);
				}
				if (this.util.findElement("[data-testid='royal_login_form']") != null)
				{
					if (!logout)
					{
						flag = true;
					...
					{
						this.util.clickByCss("[id='userNavigationLabel']", 500, 0);
						this.util.clickByCss("[class='_w0d _w0d'] [로그아웃]", 500, 0);
					}
				}
                
                this.util.sendKey("[data-testid='royal_email']", id, 300);
                this.util.sendKey("[data-testid='royal_pass']", pw, 300);
                this.util.sendEnter();
				...
				while (num < 200)
				{
					...
					else if (this.util.findElements("[data-testid='royal_login_form']").Count<IWebElement>() <= 0)
					{
						num++;
					}
					else
		...
		public string signupBand(BandInfo band, string nickName)
		{
			...
			try
			{
				...
				else
				{
					...
					{
						if (this.util.findElement("[class='header -noTitlebar -profile']") != null)
						{
							this.util.sendKey("[id='join_answer']", "네^^", 500);
							this.util.clickByCss("[class='uButton -confirm -sizeL _confirmBtn']", 500, 0);
						}
						else if (this.util.findElement("[class='main -tSpaceNone']") == null)
						{
											...
											if (this.util.findElementWithRec("[class='main -tSpaceNone']", 5) != null)
											{
												...
												if (webElements.Count != 0)
												{
													...
													else
													{
														...
														{
															this.util.findElement("[class='commentWrite _use_keyup_event']").Click();
															this.util.sendKeyPaste(chattingContent);
														}
														this.util.delay(500);
														this.util.sendEnter();
						...
						try
						{
							...
							if (flag2)
							{
								...
								else if (this.util.clickByCss("[class='postWrap'] [data-viewname='DPostLayoutView'] [class='_commentMainBtn addStatus -comment']", 1000, 0))
								{
									...
									{
										this.util.findElement("[class='commentWrite _use_keyup_event _messageTextArea']").Click();
										this.util.sendKeyPaste(commentContent);
									}
									this.util.delay(500);
                                    ...
                                    if (flag2)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            else if (this.util.findElementWithRec("[class='uButton -sizeL -confirm _btnJoinBand']", 3) == null)
                                            {
                                                ...
                                                if (this.util.clickByCss("[class='cPostWriteEventWrapper _btnOpenWriteLayer']", 1000, 0))
                                                {
                                                    ...
                                                    {
                                                        this.util.findElement("[contenteditable='true']").Click();
                                                        this.util.sendKeyPaste(postingContent);
                                                    }
                                                    this.util.delay(500);
                                                    ...
                                                    {
                                                        printLogLeft("이미지 업로드 중입니다.");
                                                        IWebElement webElement = this.util.findElement("[class='photo _btnAttachPhoto js-fileapi-wrapper'] [type='file']");
                                                        foreach (ImageFile postingImage in postingImages)
                                                        {
                                                            string str = string.Concat(postingImage.getPath(), postingImage.getFileName());
                                                            webElement.SendKeys(str.Replace('/', '\\'));
                                                        }
                                                        while (!this.util.getPageSource().Contains("uButton -confirm _submitButton"))
                                                        {
                                                            this.util.delay(100);
                                                        }
                                                        this.util.delay(500);
                                                        this.util.clickByCss("[class='footer'] [class='uButton -confirm _submitButton']", 500, 0);
